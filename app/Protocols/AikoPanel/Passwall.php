...
class Passwall
{
    ...
    public static function buildHysteria($password, $server, $sni)
    {
        ...
        if($server["version"] == 2) {
            ...
            $query = http_build_query(["protocol" => "udp", "auth" => $password, "insecure" => $server["insecure"], "peer" => $sni ?? $server["server_name"], "upmbps" => $server["down_mbps"], "downmbps" => $server["up_mbps"]]);
